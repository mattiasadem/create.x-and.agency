  ...
  try {
    ...
    const sendProgress = async (data: any) => {
      ...
      try {
        ...
        // Rethrow write errors to terminate the background generation loop
        throw error;
      }
    };
          ...
          if (editContext) {
            ...
            if (global.activeSandbox) {
              ...
              try {
                ...
                // Use the sandboxId from the context or global state
                const targetSandboxId = context?.sandboxId || global.sandboxData?.sandboxId;

                const filesResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/get-sandbox-files?sandboxId=${targetSandboxId}`, {
                  method: 'GET',
                  headers: { 'Content-Type': 'application/json' }
                });
...
Provide the complete file content without any truncation. Include all necessary imports, complete all functions, and close all tags properly.`;
        ...
        // Suppress logs for expected client-side abortions
        ...
        if (isAbortError) {
          console.log('[generate-ai-code-stream] Stream aborted by client');
          return;
        }
        ...
        if ((error as any).message?.includes('tool call validation failed')) {
          ...
          }).catch(() => { }); // Catch write error if stream is aborted
        ...
        try {
          ...
          // Ignore errors on close if the stream is already gone
        }
      }
    })();
